...
INT32 AlphaBetaSearchExact(UINT64 bk, UINT64 wh, UINT64 blank, INT32 empty,
	UINT32 *quad_parity, UINT32 color, INT32 alpha, INT32 beta, UINT32 passed, PVLINE *pline);
...
UINT64 RandomMove(UINT64 bk, UINT64 wh)
{
	...
	INT32 rnd = (INT32)(genrand_int32() % move_cnt);
	INT32 pos;
	while (rnd >= 0)
	{
...
INT32 SearchExact(UINT64 bk, UINT64 wh, UINT32 emptyNum, UINT32 color)
{
	...
	{
		HashClear(g_hash);
		eval = PVS_SearchDeepExact(bk, wh, emptyNum, color, g_hash, -INF_SCORE, INF_SCORE, NO_PASS, &line);
	}
	...
	g_pvline_len = line.cmove;
	for (int i = 0; i < g_pvline_len; i++)
	{
		g_pvline[i] = line.argmove[i];
	}
	...
	INT32 alpha, INT32 beta, INT32 passed, PVLINE *pline)
{
	...
	if (rev)
	{
		best = -SearchEmpty_1(wh ^ rev, bk ^ (pos_bit | rev), x2, &line);
	...
	if (rev)
	{
		eval = -SearchEmpty_1(wh ^ rev, bk ^ (pos_bit | rev), x1, &line);
		...
		if (quad_parity[i])
		{
			...
			　	 ここに来るのは６マス以下の空きなので、CreateMovesを呼ぶより
				...
				if (rev)
				{
					// reverse parity
					quad_parity[i] ^= 1;
					eval = -AlphaBetaSearchExact(wh ^ rev, bk ^ (pos_bit | rev), blank ^ pos_bit, 
						empty - 1, quad_parity, color ^ 1, -beta, -alpha, 0, &line);
					// restore parity
					create_quad_parity(quad_parity, blank);
					if (beta <= eval)
					{
			...
			　	 ここに来るのは６マス以下の空きなので、CreateMovesを呼ぶより
				...
				if (rev)
				{
					...
					eval = -AlphaBetaSearchExact(wh ^ rev, bk ^ (pos_bit | rev),
						blank ^ pos_bit, empty - 1, quad_parity, color ^ 1, -beta, -alpha, 0, &line);
					// restore parity
					create_quad_parity(quad_parity, blank);
					if (beta <= eval)
					{
...
#if 0
	if (g_mpcFlag && depth >= MPC_MIN_DEPTH)
	{
		if ((59 - empty) >= 36)
		{
			MPC_CUT_VAL = 1.4;
