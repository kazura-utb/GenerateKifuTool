...
UINT64 GetMoveFromAI(UINT64 bk, UINT64 wh, UINT32 emptyNum, CPUCONFIG *cpuConfig)
{
	...
	l_bk = bk;
	l_wh = wh;
	if (cpuConfig->color == WHITE) swap(&l_bk, &l_wh);

	UINT32 temp;
	// CPUはパス
	if (CreateMoves(l_bk, l_wh, &temp) == 0){
		return MOVE_PASS;
	}
	...
	// 今の局面の置換表を初期化しておく
	int key = KEY_HASH_MACRO(l_bk, l_wh, cpuConfig->color);
	...
	else
	{
		...
		g_evaluation = SearchMiddle(l_bk, l_wh, emptyNum, cpuConfig->color);
	}
		...
		for (INT32 lv = 0; lv < g_max_cut_table_size + 1; lv++)
		{
			...
			if (lv == g_max_cut_table_size)
			{
				...
				while (lower < upper)
				{
					...
					// PVS石差探索
					selectivity = lv;
					eval = PVS_SearchDeepExact(bk, wh, emptyNum, 0, parity, color, g_hash, g_pvHash, bVal - 1, bVal, NO_PASS, &selectivity, &line);
			...
			else
			{
				...
				selectivity = lv;
				eval = PVS_SearchDeepExact(bk, wh, emptyNum, 0, parity, color, g_hash, g_pvHash, aVal, bVal, NO_PASS, &selectivity, &line);
...
INT32 SearchWinLoss(UINT64 bk, UINT64 wh, UINT32 emptyNum, UINT32 color)
{
	...
	for (INT32 lv = 0; lv < g_max_cut_table_size + 1; lv++)
	{
		...
		// PVS石差探索
		eval = PVS_SearchDeepWinLoss(bk, wh, emptyNum, 0, color, g_hash, LOSS, WIN, NO_PASS, &selectivity, &line);
	...
	INT32 alpha, INT32 beta, UINT32 passed, PVLINE *pline)
{
	...
	*************************************************************/
	...
	else
	{
		...
		max = NEGAMIN;
		temp = moves & 0x8100000000000081;
		while (temp)
		{
			pos = CountBit((~temp) & (temp - 1));
